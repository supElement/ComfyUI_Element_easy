...
适用于 ComfyUI 的多参考素材面板节点，为 MiniMax-H3 / LTXV 等视频生成模型提供
首尾帧、参考图、参考视频、驱动音频等多路输入的可视化管理与剪辑能力。
...
- 需要较新的 ComfyUI（使用 comfy_api.latest 新版节点 API）。
...
| 节点 | 作用 |
|---|---|
| Element Multi REF | 可视化素材面板：导入 → 编辑 → 输出 REF_ALL_IN_ONE 信息包 |
| Element ref convert | 将信息包转换为 21 个类型化输出端口 + info 清单，供下游使用 |
...
- 取消勾选：以点选的高亮分段为输出（Shift 范围选 / Ctrl 加减选）；
- 拖蓝框两边缘调整、拖中间整体平移（视频可勾 Snap 吸附切点）。

**分段操作**：

- **Auto Split**：自动镜头切分（阈值 Thr；优先 PySceneDetect，未装则回退内置
  HSV 直方图差分）；
- **Manual（快捷键 M）**：点击轨道任意位置切一刀；
- **右键分段边界**：合并相邻两段；
- **拖动边界**：默认 Roll（两侧同时伸缩）；按住 Alt/Ctrl = 单侧修剪/延长；
- **拖动分段本体**：重排顺序（青色插入线指示落点）。

**播放与走带**：

- 传输条：|◀（片头）、◀|（段头）、▶（播当前段）、▶▶（顺序播全部段）、
  |▶（段尾）、▶|（片尾）；
- 点击标尺/音频轨/蓝框中部：拖动播放头预览（画面+音频同步）；
- Fit 适配窗口、Full 全选、Zoom 手动缩放。
...
### 3.6 视频音频联动（ref_video_audio_N）

- 导入带音频的视频到 ref_video_N 后，对应空槽 ref_video_audio_N 自动显示该视频
  波形（标注 from ref_video_N），无需单独导入；
- 输出时自动取该视频当前剪辑范围的音频；
- 右键联动槽可：打开关联视频编辑器 / 替换为独立音频。

### 3.7 状态保存

- 全部素材与编辑自动序列化进隐藏 refs_data 字段，随工作流保存/加载；
...
**Q：音频槽位没有波形？** 依赖 /element_multi_ref/media_info 解码；PyAV 解不出时
回退 ffmpeg CLI，请确认系统装有 ffmpeg。

**Q：终端出现 fps mismatch？** 标称帧率与帧数/时长偏差超 2% 时自动改用真实帧率，
属正常修正，避免越播越偏。

**Q：Auto Split 提示 fallback？** 未安装 scenedetect，已回退内置 HSV 差分，
建议 pip install scenedetect。

**Q：大视频编辑卡顿？** 帧预览（LRU 400 条）与解码帧缓存（约 600MB）已内置；
用 Fit/Zoom 控制可视范围。

**Q：加载旧工作流提示 Material missing？** 源文件被移动/删除，重新拖入即可
（上传去重不会重复存储）。

**Q：文件存在哪？** 输入素材：ComfyUI/input/element_multi_ref/（含 _index.json
去重索引）；笔刷层：ComfyUI/output/element_multi_ref/paint/。
