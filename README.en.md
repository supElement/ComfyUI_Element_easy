...
A material panel for video models (MiniMax-H3, LTXV, Wan, etc.) that require "reference images + start/end frames + reference video + audio" input. Each material can be cropped, annotated with brushes, and timed; presets can be saved—combined with queues, multiple sets of materials and prompts can be run at once without switching images.
...
- **Some functions depend on external programs**: Video export with audio requires the system to have ffmpeg installed; automatic scene detection suggests installing scenedetect; they can still be used without installation, but the corresponding functions will be limited.
...
Detailed instructions: [Chinese](Element_multi_ref_zh.md) | [English](Element_multi_ref_en.md)</sub>
...
- Each card has an independent simple image editor (cropping, brush annotations) and audio/video timeline editor (quantized alignment according to rules like 17n+5, 8n+1);
- Supports storage, recall, load, and export of multiple presets, as well as categorizing all references and prompts;
- `run_preset_NUM` parameter corresponds to the preset list index; when receiving inputs that change with prompts, combined with sampling inference nodes, it can achieve continuous generation or editing of multiple images/videos;
...
Detailed instructions: [Chinese](Element_scene_detection.zh.md) | [English](Element_scene_detection.en.md)</sub>
...
- Directly outputs segment frame sequences with precise corresponding audio, supporting export of mp4 with audio (requires ffmpeg).
...
> ⚠️ Except for Element ImageCurve, do not add points at both ends of the curve simultaneously: the curves at both ends are closed loops, adding points at both ends will make one point invalid, and in fact, one endpoint is sufficient to complete the color adjustment.
...
Browse image files under a specified path, select one to enter the editing panel for simple editing: freely draw lines, masks, boxes, circles, and crop images.

- `Shift + Left click`: Draw straight lines, squares, or circles
- `L-alpha`: Load the image's alpha channel onto the canvas
- `Return`: Switch between the zoom panel and the editing panel
- In editing mode, supports `Ctrl+V` to paste images, and supports dragging directly from a web page or file explorer
...
Mask stroke: individual control of inner and outer stroke widths and blur levels, supports adding overall weight to non-stroke areas (ensuring the output mask has no areas with a weight of 0).
...
- In constant mode: feathering controls the overall blur level, content_blur controls the blur level of the original image extension area, background_color parameter takes effect (compatible with RGB color and HEX color code)
...
**P button**: Rearrange the x-coordinates of control points—when the number of points is less than steps+1, the first n-1 points are reordered to 0, 1/steps, ..., (n-2)/steps, y remains unchanged, and the last point is kept as is; when the number of points is at least steps+1, take the first steps+1 points evenly distributed. Simply put: **Without changing the step count and step size in the front, add sampling steps in the back (to increase details)**.
...
Create a monochrome image, with color selected from the color wheel, brightness controlled by a slider; the eyedropper can pick color from any position on the screen, but if the browser is not Chrome / Edge 96+, the eyedropper may not be usable, and the color palette button next to it can be used instead.
...
<sub>New in v0.0.6</sub>

Break the input text into lines by character count, supporting punctuation avoidance at the beginning and end, punctuation compression, and hanging. The final effect depends on the output carrier.
...
The node's **video export (with audio)** feature requires the system to have ffmpeg installed. Windows users can download from [ffmpeg](https://www.gyan.dev/ffmpeg/builds/) and extract it, then add the bin directory to PATH.
