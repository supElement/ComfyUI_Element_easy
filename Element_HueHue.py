...
def compute_HueHue_logic(image, curve_data_str=None, lut_tensor=None, preview_mode=False, preview_size=512):
    if preview_mode:
    ...
    hue_shift = lut_tensor[idx0] * (1.0 - w) + lut_tensor[idx1] * w
    # 将 0.0~2.0 映射为 -0.5~+0.5 的色相环偏移（1.0为中性点）
    hue_offset = (hue_shift - 1.0) * 0.5
    new_h = (h + hue_offset) % 1.0
    new_hsv = torch.stack([new_h, s, v], dim=-1)
    ...
    def apply_curve(self, image, curve_data, preview_size, frame_index, output_mode, unique_id):
        ...
        with torch.no_grad():
            if not output_mode:
                ...
                out_tensor = compute_HueHue_logic(frame, lut_tensor=lut_tensor, preview_mode=False)
                ...
                for start in range(0, batch_size, chunk_size):
                    ...
                    processed = compute_HueHue_logic(chunk, lut_tensor=lut_tensor, preview_mode=False)
        ...
        if out_tensor.shape[0] > 0:
            ...
            filename = f"huehue_preview_{uuid.uuid4().hex[:8]}.png"
...
@server.PromptServer.instance.routes.post("/element_huehue/live_preview")
async def huehue_live_preview(request):
    try:
        ...
        with torch.no_grad():
            ...
            out_tensor = compute_HueHue_logic(
                frame_gpu,
        ...
        filename = f"huehue_live_{unique_id}.png"
