...
def compute_huesat_logic(image, curve_data_str=None, lut_tensor=None, preview_mode=False, preview_size=512):
    if preview_mode:
    ...
    sat_mult = lut_tensor[idx0] * (1.0 - w) + lut_tensor[idx1] * w
    new_s = torch.clamp(s * sat_mult, 0.0, 1.0)

    new_hsv = torch.stack([h, new_s, v], dim=-1)
...
class ElementHueSat:
    ...
    def apply_curve(self, image, curve_data, preview_size, frame_index, output_mode, unique_id):
        ...
        print(
            f"[ElementHueSat][apply_curve] input_device={image.device}, input_dtype={image.dtype}, "
        ...
        with torch.no_grad():
            if not output_mode:
                ...
                out_tensor = compute_huesat_logic(frame, lut_tensor=lut_tensor, preview_mode=False)
                ...
                for start in range(0, batch_size, chunk_size):
                    ...
                    processed = compute_huesat_logic(chunk, lut_tensor=lut_tensor, preview_mode=False)
        ...
        if out_tensor.shape[0] > 0:
            ...
            filename = f"huesat_preview_{uuid.uuid4().hex[:8]}.png"
...
@server.PromptServer.instance.routes.post("/element_huesat/live_preview")
async def huesat_live_preview(request):
    try:
        ...
        print(
            f"[ElementHueSat][live_preview] cache_device={image.device}, cache_dtype={image.dtype}, "
        ...
        with torch.no_grad():
            ...
            out_tensor = compute_huesat_logic(
                frame_gpu,
        ...
        filename = f"huesat_live_{unique_id}.png"
