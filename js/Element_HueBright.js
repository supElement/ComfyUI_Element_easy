...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name !== "ElementHueBright") return;
        ...
        try {
            ...
            console.warn("[HueBright] nodeData hide patch failed:", e);
        }
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            titleSpan.innerText = "Hue vs Brightness";
            ...
            loadBtn.onclick = async () => {
                try {
                    ...
                    if (!prompt || typeof prompt !== "object") {
                        console.error("[HueBright] graphToPrompt() returned invalid prompt:", p);
                        return;
                    }
                    ...
                    if (Object.keys(isolatedPrompt).length === 0) {
                        console.warn("[HueBright] isolatedPrompt is empty. selectedNodeId =", selectedNodeId);
                        return;
                    }
                    ...
                    if (!resp.ok) {
                        ...
                        console.error("[HueBright] /prompt failed:", resp.status, txt);
                    }
                } catch (err) {
                    console.error("[HueBright] Failed to execute isolated node:", err);
                }
            };
            ...
            const updateLivePreview = (isDragging = false) => {
                ...
                (async () => {
                    try {
                        const response = await api.fetchApi("/element_huebright/live_preview", {
                            method: "POST",
