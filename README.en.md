...
## v1.4.2
  ...
  - When merging cropped images into the original image, if conditions allow, the most reliable merging method is: original_image + edited_crop_B + original_crop_A. Here, original_crop_A is the unmodified or deformed image cut from original_image.
  ...
  - Adds the color matching mode Adaptive Local (strong), which can better repair the color difference after the image is edited. Three parameters are added specifically for this mode: adapt_thresh (color difference threshold), adapt_kernel (blurring kernel, simply understood as feathering), adapt_align (pre-matching strength - corrects the local color spots generated after the image is merged, the higher the value, the more obvious the correction, but it has a negative impact on the overall color correction of the image). In this mode, the feather_amount parameter is invalid.

  ![image](https://github.com/user-attachments/assets/88a429b5-18d8-429a-994a-db9c62791cc8)

## v1.4.0

  - Added an optional image input port to the LoadImage_Preview node, which can be used to bridge the preview of the editor; optimized the layout of the editing panel.
...
## v1.3.0
  ...
  - Note: Do not add points at both ends of the curve at the same time, as this will not cause an error, but it will make the other point invalid. Only one endpoint is needed for the upper end point to complete the color adjustment, because the curves at both ends are closed loops.
...
## v1.2.4 （fix in V1.3.8）
  ...
  - Click the position on the curve with the mouse to add or remove control points (double-click to add, right-click to delete), and keep other control points unchanged when adding or removing points. Remove the X-axis movement restriction of control points. 
...
## v1.1.3
  ...
  The initial purpose of the ChessboardPattern node was to suppress the pixel offset problem after style transformation in qwenEdit, first transform the mask area style, then transform the invert mask area (but it needs two samplings, this method does not work in the Klein model), and then it was found that the ChessboardPattern mask affects the model's recognition of objects, so the Black White Color node was created. Try to make the area of the black and white regions average to reduce the problem of inconsistent hue. 
...
## v0.0.7
  ...
  Added Mask Stroke node, mask outlining, supports separate control of the width and blur of inner and outer strokes, and supports adding weight to the non-stroked area as a whole (so that the output mask does not have an area with weight of 0).
...
Empty Image RGB：Supports RGB and 16-bit color information input, the node will automatically identify the color information color_code. Outputs a solid color image, with an image size output port, the image size will be approximated based on divisible_by (divisible).
...
Random Chars (Append)： Adds invalid special characters to the input text (characters and number can be customized), mainly to make the generated image have more changes without destroying the prompt intention (unknown whether it works), because the random changes in the composition of the image generated by qwen_image and Z_image models are small. Here "invalid" is relative.
                       Can simply set the insertion position (before, end, insert), where insert is to evenly distribute the number of characters inserted into the punctuation marks behind the original text, from the end to the beginning.
