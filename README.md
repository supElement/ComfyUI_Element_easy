...
- 为 MiniMax-H3、ltx、wan、klein、qwen等需要多路参考输入的视频或图像推理模型设计的素材及提示词管理方案；
- 配有简易图像编辑器和音视频编辑器，每个参考卡片的编辑器参数独立；
- 支持多个预设的存储、调用、载入、导出以及收集归类所有参考及提示词。
- run_preset_NUM 参数值对应预设列表中的序号，当接收comfyUI随提示词队列变化的输入时，配合采样推理节点，以达到连续生成或编辑多个图像或视频的目的。
