...
⚠️ 注意：若加载了预设，`run_preset_NUM` 参数会自动变为预设的序号，此时若修改了参考区域的内容，必须将`run_preset_NUM` 参数设置为 0 （输出当前界面），或存储/另存预设，否则输出的结果将是没有修改之前的，这是正常行为。
...
- 快捷键：空格键 = 播放/停止 所有片段；鼠标滚轮 = 缩放时间线；shift+鼠标滚轮 = 左右滑动时间线显示范围；分段选择：Ctrl+点击 = 加选，Shift+点击 = 范围连选；修剪片段：按住 Alt/Ctrl 拖动 = 联动修剪；delete/Backspace 删除选择的分段。
