...
面板底栏左下角有 4 个区域开关图标（参考图 / 首尾帧 / 音视频 / Prompt）：你可以自由组合面版，以应对不同的需求，甚至可以将这个节点当成提示词预设节点来使用。

- 点击切换对应区域的显示与隐藏；隐藏后图标变半透明并带红色斜线；
- 开关状态随工作流保存，也保存在每个预设快照中；
- 开关只影响面板显示，不影响数据：隐藏区域的已填槽位照常参与运行输出；
- 自适应规则：
  - 隐藏 Prompt 后输入框消失，底部按钮栏保持原位不动，腾出的空间全部给上方参考区；
  - 只保留 Prompt 时参考区整体折叠，prompt 输入框占满全高；
...
| ⏮ ◀ ▶ ⏭ | 应用第一个 / 上一个 / 下一个 / 最后一个预设 |
| Presets | 打开预设弹窗 |
| 软盘 | Save Preset |
| 文件夹 | 导出文件夹设置（Export folder）|
| 包裹 | Collect and Export |

顶栏（标题 · 操作提示 · 状态反馈 · Clear）固定单行：节点过窄时先隐藏 Drop files… 提示，反馈信息过长自动截断为 …；标题与 Clear 永不隐藏。
